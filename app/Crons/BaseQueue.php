...
abstract class BaseQueue extends Base {
    ...
    protected function baseProcess(string $action): void {
        ...
        switch ($action) {
            case \Wolf\Utils\Constants::get()->DELETE_USER_QUEUE_ACTION_TYPE:
                $prefix = 'Deletion';
                break;
            case \Wolf\Utils\Constants::get()->BLACKLIST_QUEUE_ACTION_TYPE:
                $prefix = 'Blacklist';
                break;
            case \Wolf\Utils\Constants::get()->ENRICHMENT_QUEUE_ACTION_TYPE:
                $prefix = 'Enrichment';
                break;
            case \Wolf\Utils\Constants::get()->RISK_SCORE_QUEUE_ACTION_TYPE:
                $prefix = 'Risk score';
                break;
        }
        ...
        while (!$bottom) {
            ...
            foreach ($batch as $item) {
                ...
                $batchTimeout = (time() - $start) > \Wolf\Utils\Constants::get()->ACCOUNT_OPERATION_QUEUE_EXECUTE_TIME_SEC;
                if ($batchTimeout) {
                    break;
                }
            }
            // exit if took too long
            $bottom = (time() - $start) > \Wolf\Utils\Constants::get()->ACCOUNT_OPERATION_QUEUE_EXECUTE_TIME_SEC;
        }
        ...
        if (count($errors)) {
            ...
            \Wolf\Utils\ErrorHandler::saveErrorInformation(\Base::instance(), $errObj);
        }
