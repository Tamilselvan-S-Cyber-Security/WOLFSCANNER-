...
class NotificationsHandler extends Base {
    public function process(): void {
        $model = new \Wolf\Models\NotificationPreferences();
        ...
        foreach ($operators as $operator) {
            if (\Wolf\Utils\Cron::checkTimezone($operator['timezone'] ?? '')) {
                try {
                    $name   = $operator['firstname'] ?? '';
                    $email  = $operator['email'] ?? '';
                    $review = $operator['review_queue_cnt'] ?? 0;
                    if (!\Wolf\Utils\Cron::sendUnreviewedItemsReminderEmail($name, $email, $review)) {
