...
namespace Wolf\Controllers\Admin\Context;

class Data extends \Wolf\Controllers\Base {
    private \Wolf\Models\Context\User $userModel;
    private \Wolf\Models\Context\Ip $ipModel;
    private \Wolf\Models\Context\Device $deviceModel;
    private \Wolf\Models\Context\Email $emailModel;
    private \Wolf\Models\Context\Phone $phoneModel;
    private \Wolf\Models\Context\Event $eventModel;
    private \Wolf\Models\Context\Session $sessionModel;
    private \Wolf\Models\ApiKeys $keyModel;

    private ?\Wolf\Assets\Context $extraModel;
    ...
    public function __construct() {
        $this->userModel    = new \Wolf\Models\Context\User();
        $this->ipModel      = new \Wolf\Models\Context\Ip();
        $this->deviceModel  = new \Wolf\Models\Context\Device();
        $this->emailModel   = new \Wolf\Models\Context\Email();
        $this->phoneModel   = new \Wolf\Models\Context\Phone();
        $this->eventModel   = new \Wolf\Models\Context\Event();
        $this->sessionModel = new \Wolf\Models\Context\Session();
        $this->keyModel     = new \Wolf\Models\ApiKeys();

        $this->extraModel   = \Wolf\Utils\Assets\ContextClass::getContextObj();

        $this->suspiciousWordsUrl       = \Wolf\Utils\Assets\Lists\Url::getList();
        $this->suspiciousWordsUserAgent = \Wolf\Utils\Assets\Lists\UserAgent::getList();
        $this->suspiciousWordsEmail     = \Wolf\Utils\Assets\Lists\Email::getList();
    }
    ...
    public function getContext(array $accountIds, int $apiKey): array {
        ...
        $utcTime            = new \DateTime('now', \Wolf\Utils\Timezones::getUtcTimezone());
        $timezone           = \Wolf\Utils\Timezones::getTimezone($timezoneName);
    ...
    private function extendParams(array &$record): void {
        ...
        $record['le_with_long_local_part_length']   = $localPartLen > \Wolf\Utils\Constants::get()->RULE_EMAIL_MAXIMUM_LOCAL_PART_LENGTH;
        $record['le_with_long_domain_length']       = $domainPartLen > \Wolf\Utils\Constants::get()->RULE_EMAIL_MAXIMUM_DOMAIN_LENGTH;
        $record['le_email_in_blockemails']          = $record['le_blockemails'] ?? false;
        $record['le_is_invalid']                    = $record['le_exists'] && !\Wolf\Utils\Conversion::filterEmail($record['le_email']);
        ...
        $record['eup_has_rare_browser'] = (bool) count(array_diff($record['eup_browser_name'], array_keys(\Wolf\Utils\Constants::get()->RULE_REGULAR_BROWSER_NAMES)));
        $record['eup_has_rare_os']      = (bool) count(array_diff($record['eup_os_name'], \Wolf\Utils\Constants::get()->RULE_REGULAR_OS_NAMES));
    ...
    private function extendEventParams(array &$record): void {
        ...
        //$accountLoginFailId = \Wolf\Utils\Constants::get()->ACCOUNT_LOGIN_FAIL_EVENT_TYPE_ID;
        $accountEmailChangeId               = \Wolf\Utils\Constants::get()->ACCOUNT_EMAIL_CHANGE_EVENT_TYPE_ID;
        $accountPwdChangeId                 = \Wolf\Utils\Constants::get()->ACCOUNT_PASSWORD_CHANGE_EVENT_TYPE_ID;
        ...
        $record['event_http_method_head']   = in_array(\Wolf\Utils\Constants::get()->EVENT_REQUEST_TYPE_HEAD, $record['event_http_method']);
    ...
    private function getDaysDiff(?string $dt1, ?string $dt2): int {
        ...
        return \Wolf\Utils\Conversion::intVal($diff, 0);
    }
