...
class Cron extends \Prefab {
        ...
        foreach ($expressionParts as $i => $field) {
            ...
            foreach ($fieldParts as $part) {
                ...
                if ($start === '*') {
                    ...
                    $start = \Wolf\Utils\Conversion::intValCheckEmpty($start, 0);
                    $end = \Wolf\Utils\Conversion::intValCheckEmpty($end, $start);
                }
                $step = \Wolf\Utils\Conversion::intValCheckEmpty($step, 0);
    ...
    public static function parseTimestamp(\DateTime $time): array {
        return [
            \Wolf\Utils\Conversion::intValCheckEmpty($time->format('i'), 0), // minute
            \Wolf\Utils\Conversion::intValCheckEmpty($time->format('H'), 0), // hour
            \Wolf\Utils\Conversion::intValCheckEmpty($time->format('d'), 1), // day of month
            \Wolf\Utils\Conversion::intValCheckEmpty($time->format('m'), 1), // month
            \Wolf\Utils\Conversion::intValCheckEmpty($time->format('w'), 0), // day of week
        ];
    }
    ...
    private function route(): void {
        ...
        $this->f3->set('ONERROR', \Wolf\Utils\ErrorHandler::getCronErrorHandler());
        ...
        \Wolf\Utils\Updates::syncUpdates();
    ...
    public function execute(string $jobName): void {
        ...
        $instance->$method();
        \Wolf\Utils\Cron::printLogs($instance->getLog());
    }
