...
abstract class Base {
    ...
    public function __construct() {
        ...
        if (!\Wolf\Utils\Database::initConnect(boolval($keepSessionInDb))) {
            $this->f3->error(404);
        }

        //Determine current user
        \Wolf\Utils\Routes::setCurrentRequestOperator();
        \Wolf\Utils\Routes::setCurrentRequestApiKey();
    ...
    public function validateCsrfToken(): int|bool {
        ...
        $token = \Wolf\Utils\Conversion::getStringRequestParam('token');

        if (!isset($token) || $token === '' || !isset($csrf) || $csrf === '' || $token !== $csrf) {
            return \Wolf\Utils\ErrorCodes::CSRF_ATTACK_DETECTED;
        }
