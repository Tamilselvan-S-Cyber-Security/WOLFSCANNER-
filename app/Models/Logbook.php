...
class Logbook extends \Wolf\Models\BaseSql {
    ...
    public function getLastSucceededEvent(int $apiKey): array {
        $params = [
            ':api_key'          => $apiKey,
            ':endpoint'         => '/sensor/',
            ':success'          => \Wolf\Utils\Constants::get()->LOGBOOK_ERROR_TYPE_SUCCESS,
            ':validation_error' => \Wolf\Utils\Constants::get()->LOGBOOK_ERROR_TYPE_VALIDATION_ERROR,
        ];
    ...
    public function rotateRequests(?int $apiKey): int {
        $params = [
            ':key'      => $apiKey,
            ':limit'    => \Wolf\Utils\Variables::getLogbookLimit(),
        ];
