...
class Events extends \Wolf\Models\BaseSql {
    ...
    protected function applyLimit(string &$query, array &$queryParams): void {
        $start = \Wolf\Utils\Conversion::getIntRequestParam('start');
        $length = \Wolf\Utils\Conversion::getIntRequestParam('length');
    ...
    public function retentionDeletion(int $weeks, int $apiKey): int {
        ...
        $params = [
            ':api_key'  => $apiKey,
            ':weeks'    => $weeks,
            ':week_sec' => \Wolf\Utils\Constants::get()->SECONDS_IN_WEEK,
        ];
