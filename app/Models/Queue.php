...
class Queue extends \Wolf\Models\BaseSql {
    ...
    public function setWaiting(array $ids): void {
        $this->setStatus(\Wolf\Utils\Constants::get()->WAITING_QUEUE_STATUS_TYPE, $ids);
    }

    public function setFailed(array $ids): void {
        $this->setStatus(\Wolf\Utils\Constants::get()->FAILED_QUEUE_STATUS_TYPE, $ids);
    }

    public function setCompleted(array $ids): void {
        $this->setStatus(\Wolf\Utils\Constants::get()->COMPLETED_QUEUE_STATUS_TYPE, $ids);
    }

    public function setExecuting(array $ids): void {
        $this->setStatus(\Wolf\Utils\Constants::get()->EXECUTING_QUEUE_STATUS_TYPE, $ids);
    }

    public function isInQueueStatus(int $accountId, string $action, int $key): array {
        $params = [
            ':account'  => $accountId,
            ':status'   => \Wolf\Utils\Constants::get()->COMPLETED_QUEUE_STATUS_TYPE,
            ':key'      => $key,
            ':action'   => $action,
        ];
    ...
    public function actionIsInQueueProcessing(string $action, int $key): bool {
        $params = [
            ':failed'       => \Wolf\Utils\Constants::get()->FAILED_QUEUE_STATUS_TYPE,
            ':completed'    => \Wolf\Utils\Constants::get()->COMPLETED_QUEUE_STATUS_TYPE,
            ':key'          => $key,
            ':action'       => $action,
        ];
    ...
    public function addBatch(array $accounts, string $action): void {
        ...
        $params = [
            ':action'   => $action,
            ':waiting'  => \Wolf\Utils\Constants::get()->WAITING_QUEUE_STATUS_TYPE,
        ];
    ...
    public function setFailedForStuckAction(string $action): void {
        $params = [
            ':action'   => $action,
            ':status'   => \Wolf\Utils\Constants::get()->FAILED_QUEUE_STATUS_TYPE,
            ':stuck'    => \Wolf\Utils\Constants::get()->EXECUTING_QUEUE_STATUS_TYPE,
        ];
    ...
    public function checkExecuting(string $action): array {
        $params = [
            ':action'   => $action,
            ':status'   => \Wolf\Utils\Constants::get()->EXECUTING_QUEUE_STATUS_TYPE,
        ];
    ...
    public function getNextBatchInQueue(string $action, int $size): array {
        $params = [
            ':batch'    => $size,
            ':action'   => $action,
            ':status'   => \Wolf\Utils\Constants::get()->WAITING_QUEUE_STATUS_TYPE,
        ];
