...
class DateRange {
    ...
    public static function isQueueTimeouted(string $updated): bool {
        return !self::inIntervalTillNow($updated, \Wolf\Utils\Constants::get()->ACCOUNT_OPERATION_QUEUE_AUTO_UNCLOG_AFTER_SEC);
    }
    ...
    public static function getDatesRangeFromRequest(int $offset = 0): ?array {
        $dates      = null;
        $dateTo     = \Wolf\Utils\Conversion::getStringRequestParam('dateTo', true);
        $dateFrom   = \Wolf\Utils\Conversion::getStringRequestParam('dateFrom', true);
        $keepDates  = \Wolf\Utils\Conversion::getIntRequestParam('keepDates', true);
    ...
    public static function getResolutionFromRequest(): string {
        $resolution = \Wolf\Utils\Conversion::getStringRequestParam('resolution', true) ?? 'day';

        return array_key_exists($resolution, \Wolf\Utils\Constants::get()->CHART_RESOLUTION) ? $resolution : 'day';
    }
