...
class RulesClasses extends Base {
    ...
    public static function getUserScoreClass(?int $score): array {
        ...
        if ($score >= \Wolf\Utils\Constants::get()->USER_LOW_SCORE_INF && $score < \Wolf\Utils\Constants::get()->USER_LOW_SCORE_SUP) {
            $cls = 'low';
        }

        if ($score >= \Wolf\Utils\Constants::get()->USER_MEDIUM_SCORE_INF && $score < \Wolf\Utils\Constants::get()->USER_MEDIUM_SCORE_SUP) {
            $cls = 'medium';
        }

        if ($score >= \Wolf\Utils\Constants::get()->USER_HIGH_SCORE_INF) {
            $cls = 'high';
        }
    ...
    protected static function getNamespace(bool $core = true): string {
        return $core ? '\\Wolf\\Rules\\Core' : '\\Wolf\\Rules\\Custom';
    }
    ...
    public static function getSingleRuleObject(string $uid, ?\Ruler\RuleBuilder $ruleBuilder): ?\Wolf\Assets\Rule {
        $obj = null;
        $cores = [false, true];
