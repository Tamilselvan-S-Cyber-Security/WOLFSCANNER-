...
class Timezones {
    ...
    public static function getOperatorOffset(?\Wolf\Entities\Operator $operator): int {
    ...
    public static function getCurrentOperatorOffset(): int {
        return self::getOperatorOffset(\Wolf\Utils\Routes::getCurrentRequestOperator());
    }
    ...
    public static function getLastNDaysRange(int $days = 1, int $offset = 0): array {
        $now = time();
        $daySeconds = \Wolf\Utils\Constants::get()->SECONDS_IN_DAY;
    ...
    public static function getPreviousWeekRange(int $offset = 0): array {
        ...
        $date->setTime(0, 0, 0);
        $dow = \Wolf\Utils\Conversion::intValCheckEmpty($date->format('N'), 1);
    ...
    public static function getWeekAgoDayRange(int $offset = 0): array {
        ...
        $week = \Wolf\Utils\Constants::get()->SECONDS_IN_WEEK;
        $day = \Wolf\Utils\Constants::get()->SECONDS_IN_DAY;
    ...
    public static function getCurWeekRange(int $offset = 0): array {
        ...
        $date->setTimestamp($now + $offset);
        $date->setTime(0, 0, 0);
        $dow = \Wolf\Utils\Conversion::intValCheckEmpty($date->format('N'), 0);
        $day = \Wolf\Utils\Constants::get()->SECONDS_IN_DAY;
    ...
    public static function timezonesList(): array {
        $utcTime = new \DateTime('now', self::getUtcTimezone());
        $timezones = \Wolf\Utils\Variables::getAvailableTimezones();
