...
use Wolf\Utils\SystemMessages;
use PHPUnit\Framework\TestCase;

/**
 * Unit tests for Wolf\Utils\SystemMessages.
 *
...
final class SystemMessagesTest extends TestCase {
    public function testSyslogLineBuildsPriTimestampHostAppPidAndMessage(): void {
        ...
        // App and pid are embedded as "app[PID]:" with fixed host "Wolf Security scanner".
        $this->assertStringContainsString(' Wolf Security scanner ' . $app . '[' . $pid . ']: ', $line);
        ...
        $this->assertMatchesRegularExpression(
            '/^<\d+>[A-Z][a-z]{2}\s+\d{1,2}\s+\d{2}:\d{2}:\d{2}\s+Wolf\s+/',
            $line
        );
    }
